
# Market Risk Monitor

![Market Regime](https://img.shields.io/badge/Market%20Regime-Recovery-green)

**🟢 Recovery**  
**Score:** Downturn 0/3 | Recovery 3/3  
**Last Updated:** 2026-06-26

---

⚠️ **Disclaimer**

This is an automated market signal summary for informational purposes only.
It is not financial advice.

A note from the author:
There are hundreds of resources on the Internet in addition to learning resources available through your investment platform.
For example, [this one by Ramit Sethi](https://youtu.be/FF5-FbhaAyc?si=52cXbGUBFqxifu7Q), or [this one by Jaspreet Singh](https://youtu.be/qdqLIjszqy4?si=-R0Sa7C_Q0bCHY08), or [this one by Erin Moriarity](https://youtu.be/FYMfX3Aljow?si=MPQ7nICG0nA1U6vh), or articles like [this one by Fidelity](https://www.fidelity.com/learning-center/smart-money/roth-ira-taxes).
Seek out the information you need for your future self!

---

## AI Risk Commentary

**Risk commentary:**  
Market conditions remain constructive in a **Recovery** regime, with equity trends holding up and volatility still contained. Yield levels remain elevated, which keeps income competition from bonds meaningful, but overall risk appetite is still supported. Conditions appear **stable** for now.

- **SPY** is trending higher: price **734.3** vs **200-day MA 690.14**, showing an uptrend.
- **QQQ** is also strong: price **716.38** vs **100-day MA 650.97**.
- **VIX** is **18.89**, indicating moderate but not stressed volatility.
- **TNX** is **4.37%**, so yields remain relatively high and supportive of fixed-income returns.
- **Mortgage rate** is **6.49%**, with a **Neutral** condition.
- **Income spread**: **S&P dividend yield 1.03% vs 10Y yield 4.4%**, spread **-3.37%**. This **favors bonds over equities** on income.
- **OVX** is **46.96** with a **low** regime, suggesting calmer oil volatility.
- **ARKK** is up **13.58% over 3 months**, reinforcing the recovery/risk-on tone.

Raw data is available in **/data**.

---

## Charts

### SPY Trend
![SPY](charts/spy.png)

### QQQ Trend
![QQQ](charts/qqq.png)

### ARKK Drawdown
![ARKK](charts/arkk.png)

### VIX
![VIX](charts/vix.png)

### 10Y Yield
![TNX](charts/tnx.png)

### Oil Volatility
![OVX](charts/ovx.png)

### Mortgage Conditions
![Mortgage](charts/mortgage.png)

### SPY Trailing Dividend Yield (proxy)
![Income Spread](charts/income_spread.png)

---

[View raw data](data/market_snapshot.json)

---

## RSS Feed

https://kam-reef.github.io/market-summary/feed.xml

---

## Data

- Signals: [data/signals.json](data/signals.json)  
- History: [data/history.json](data/history.json)
